...
# مخازن الحالات المؤقتة
user_states = {}       # حالات إدخال الرقم والكود بالبوت
active_web_tokens = {} # توكنات لوحة التحكم للويب {token: session_path}
...
# الواجهة الرسمية لتطبيق "وهم" (HTML/CSS/JS مدمجة)
WEB_TEMPLATE = """
<!DOCTYPE html>
...
<head>
    ...
    <title>وهم | Wahm Web</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background-color: #0f172a; color: #f8fafc; font-family: system-ui, -apple-system, sans-serif; }
        .sidebar { background-color: #1e293b; border_left: 1px solid #334155; }
        ...
        .chat-item.active { background-color: #0284c7; }
...
<body class="h-screen flex overflow-hidden">
    ...
    <!-- منطقة عرض الرسائل والمحادثة النشطة -->
    <div class="hidden md:flex flex-1 flex-col h-full chat-area">
        ...
        <div id="messages-container" class="flex-1 p-4 overflow-y-auto space-y-3 flex flex-col justify-center items-center">
            <div class="text-center text-slate-500 max-w-sm">
                ...
                <p class="text-xs">تصفح حسابك وقنواتك ومحادثاتك بالكامل مباشرة من هذه اللوحة.</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        if (!token) {
            document.body.innerHTML = '<div class="m-auto text-red-400 font-bold">❌ رابط غير صالح أو التوكن مفقود!</div>';
        } else {
            loadAccountData();
            loadDialogs();
        }
        ...
        async function loadDialogs() {
            try {
                ...
                document.getElementById('chats-list').innerHTML = '<div class="p-4 text-center text-red-400">حدث خطأ أثناء تحميل البيانات</div>';
            }
        }
        ...
        function renderDialogs() {
            ...
            if (filtered.length === 0) {
                container.innerHTML = '<div class="p-8 text-center text-slate-500 text-sm">لا توجد محادثات في هذا القسم</div>';
                return;
            }
...
@app.route('/')
def home():
    # صفحة الفحص الافتراضية
    return "<h1>Wahm Server Active 🚀</h1>", 200

@app.route('/dashboard')
def dashboard():
    return render_template_string(WEB_TEMPLATE)

# === API للتحكم وتغذية الواجهة بالبيانات الحية ===
...
async def api_me():
    ...
    if token not in active_web_tokens:
        return jsonify({"success": False, "error": "التوكن غير صالح"}), 403
...
async def api_dialogs():
    token = request.args.get('token')
    if token not in active_web_tokens:
        return jsonify({"success": False, "error": "التوكن انتهى أو غير صالح"}), 403
...
async def api_messages():
    ...
    if token not in active_web_tokens or not chat_id:
        return jsonify({"success": False, "error": "طلب غير مكتمل"}), 400
    ...
    try:
        ...
        async for msg in client.iter_messages(target, limit=25):
            sender = "أنا" if msg.out else ("الطرف الآخر")
            messages_data.append({
                "id": msg.id,
                "text": msg.text or "[محتوى وسائط/غير نصي]",
                "out": msg.out,
                "sender": sender,
...
# =============================================================
# 3. بوت تليجرام واستخراج الجلسة ورابط "وهم"
# =============================================================
bot = TelegramClient('wahm_bot_session', API_ID, API_HASH).start(bot_token=BOT_TOKEN)
...
async def start_command(event):
    ...
    await event.respond(
        "👻 **مرحباً بك في تطبيق وَهَم**\n\nاضغط على الزر أدناه لمشاركة رقمك واستخراج لوحة التحكم والجلسة الخاصة بك:", 
        buttons=phone_btn
    )
    ...
    if user_id in user_states and user_states[user_id]["step"] == "WAITING_PHONE":
        if event.message.contact:
            ...
            try:
                ...
                await event.respond(
                    f"✅ تم إرسال كود التحقق.\n\nأدخل الكود عبر اللوحة الشفافة أدناه:\n\n{disp_text}", 
                    buttons=btns
                )
                
            except Exception as error:
                await temp_client.disconnect()
                await event.respond(f"❌ حدث خطأ: {str(error)}")
...
async def process_keyboard(event):
    ...
    if user_id not in user_states or user_states[user_id]["step"] != "WAITING_CODE":
        await event.answer("⚠️ انتهاء صلاحية الجلسة، أرسل /start", alert=True)
        return
    ...
    await event.edit(f"✅ أرسلنا الكود إلى حسابك.\n\nأدخله بالأسفل:\n\n{disp_text}", buttons=btns)
    await event.answer()
...
async def verify_code_and_login(event, user_id):
    ...
    if not code:
        await event.answer("⚠️ أدخل الكود أولاً!", alert=True)
        return

    await event.edit("⏳ جاري التحقق وإنشاء واجهة التحكم...")

    try:
        await client.sign_in(phone, code, phone_code_hash=code_hash)
        await generate_web_and_session(user_id)

    except SessionPasswordNeededError:
        state["step"] = "WAITING_2FA"
        await event.edit("🔐 **الحساب محمي بـ (2FA).**\n\nأرسل كلمة السر كرسالة نصية هنا:")
        
    except (PhoneCodeInvalidError, PhoneCodeExpiredError):
        state["code"] = ""
        disp_text, btns = make_numeric_keyboard("")
        await event.edit(f"❌ الكود غير صحيح. حاول مجدداً:\n\n{disp_text}", buttons=btns)
...
async def process_2fa_password(event):
    ...
    if user_id in user_states and user_states[user_id]["step"] == "WAITING_2FA":
        ...
        try:
            ...
            await event.respond("❌ كلمة السر غير صحيحة، حاول مجدداً:")

async def generate_web_and_session(user_id):
    state = user_states[user_id]
    client = state["client"]
    ...
    if os.path.exists(file_path):
        ...
        await bot.send_file(
            ...
            caption=(
                ...
                f"📂 تم إرفاق ملف الجلسة (`.session`).\n\n"
                f"🌐 **رابط التحكم الخاص بك في تطبيق (وَهَم):**\n{wahm_link}"
            ),
            buttons=web_btn
        )
...
if __name__ == '__main__':
    ...
    print("🚀 تطبيق 'وهم' والبوت يعملان بنجاح...")
